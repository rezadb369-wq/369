...
## قانون ۷ — تم رسمیِ استاندارد (۲۰۲۶-۰-۲۸)
- هویت سایت «رسمی» است: خاکستری خنثی + یک رنگ برند (آبی اداری). هویت لوکس
  طلایی/لاجورد (v110) کامل حذف شد؛ هاله، گرادیان رنگی، بافت و بلور شیشه‌ای ممنوع.
- فایل‌های `theme-night.css` / `theme-light.css` فقط «توکن» نگه می‌دارند؛ هر
  override ساختاری یا هگز مستقیم در آن‌ها = متن محو در یکی از دو حالت = خطای
  تست (`tools/test_appearance.py` الان این را می‌گیرد).
- رنگ جدید فقط به‌صورت توکن معنایی در `tokens.css` با کنتراست AA در «هر دو»
  حالت؛ ممیزی `tools/contrast_audit.py` باید ۰ خطا بدهد.
- کف متن ۱۲px در هر دو حالت؛ تعویض تم با محو شدن ۳۰۰ms؛ بازدید اول از تمایل
  سیستم؛ ترجیح قدیمی «luxury» هم پذیرفته می‌شود.
...
| ۲۰۲۶-۰۸-۲۸ | ۷) تم رسمیِ استاندارد: توکن‌محور، AA در هر دو حالت، بدون لوکس |
...
## قانون ۶ — بازسازی به‌جای وصله وقتی به نتیجه نمی‌رسد (۲۰۲۶-۰۸-۲۶)
